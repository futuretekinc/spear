...
{
	printk(KERN_INFO "%s, Mali DRM probe, driver name: %s, version %d.%d\n", __FILE__, DRIVER_NAME, DRIVER_MAJOR, DRIVER_MINOR);
...
{
	int result = 0;
	result = platform_driver_register( &platform_drm_driver );
	printk(KERN_INFO "%s : mali drm driver platform init, result= %d (0x%08X)\n", __FILE__, result, result);
	return result;
}
